...
o=OptionParser.new do |opts|
   ...
   opts.on( '-u', '--user USERNAME', 'The username to login with' ) {|user| options[:username] = user}
   opts.on( '-p', '--password PASSWORD', 'The password to login with. 
                                     leave blank for interactive' ) {|password| options[:password] = password}
   opts.on( '-r', '--report REPORTNAME', 'The report to download' ) {|report| options[:report] = report}
